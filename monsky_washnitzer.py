...
#        return self._diff
    ...
    def coeffs(self, R=None):
        ...
        #print 'ydegree = ', y_degree
        coeffs = []
        n = y_degree - y_offset + 1
        #print 'n = ', n
        #print 'self._f.list(): ', self._f.list()
        for a in self._f.list():
            #k = a.valuation() - y_offset
            k = a.valuation()
            if k is Infinity:
                coeffs.append([zero]*n)
            #if k is Infinity: k = 0  #THIS INTRODUCED A BUG! FIXED 29 Mar 2016 as above
            else:
                k -= y_offset
                z = a.list()
                coeffs.append([zero] * k + z + [zero]*(n - len(z) - k))
        #print 'coeffshere', coeffs
        #print 'self.parent().degree(): ', self.parent().degree()
